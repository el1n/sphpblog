...
function page_content()
{
	...
	?>
		...
		<form action='add_link.php' method="post" name="addlink" id="addlink">
			<input type="hidden" name="link_id" value="<?php if(isset($link_id)){
		echo($link_id);
	}?>">
	...
	?>
			...
			<input type="text" name="link_name" id="link_name" autocomplete="OFF" value="<?php if(isset($link_name)){
		echo($link_name);
	}?>"><br /><br />
			
			<label for="link_url"><?php echo(_sb('link_url'));?></label><br />
			<input type="text" name="link_url" id="link_url" autocomplete="OFF" size="45" value="<?php if(isset($link_url)){
		echo($link_url);
	}else{
		echo('http://');
	}?>"><br /><br />
			
			<input type="submit" name="submit" value="&nbsp;<?php if(isset($link_id)){
		echo _sb('add_link_submit_btn_edit');
	}else{
		echo _sb('add_link_submit_btn_add');
	}?>&nbsp;" onclick="this.form.action='add_link.php';" />
		</form>
