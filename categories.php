...
function phpValidate($str)
{
	if($str == ''){
		...
		}elseif(strstr($str,urldecode('%0A')) !== false){

			// Unix / Mac IE
			$return_char = urldecode('%0A');
		}elseif(strstr($str,urldecode('%0D')) !== false){
...
function page_content()
{
	...
	echo($theme_vars['max_image_width']);?>px;" id="category_list" name="category_list" rows="20" cols="50" autocomplete="OFF"><?php
			$catArray = get_category_array();
	if(count($catArray) > 0){
		$str = "";
		for($i = 0;$i < count($catArray);$i++){
