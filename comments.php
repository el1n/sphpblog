...
if(array_key_exists('y',$_GET) && array_key_exists('m',$_GET) && array_key_exists('entry',$_GET)){
	...
	if(strposa($_GET['y'],array('/','.','\\','%')) === false && strlen($_GET['y']) == 2 && strposa($_GET['m'],array('/','.','\\','%')) === false && strlen($_GET['m']) == 2 && strposa($_GET['entry'],array('/','.','\\','%')) === false && strlen($_GET['entry']) == 18){
...
function page_content()
{
	...
	echo($theme_vars['max_image_width']);?>px;" id="comment_text" name="comment_text" rows="20" cols="50" autocomplete="off"></textarea><br /><br />
	...
	}elseif($blog_config->getTag('BLOG_ENABLE_CAPCHA') == 0){
	...
	}elseif($blog_config->getTag('BLOG_ENABLE_COMMENTS') == false){
		ob_end_clean();

		// Don't show anything to do with the comment editing.
		$entry_array['entry'] = _sb('nocomments');
	}elseif(are_comments_expired($GLOBALS['month'],$GLOBALS['day'],$GLOBALS['year'])){

		// Check Expiration Date
		ob_end_clean();
		$entry_array['entry'] = _sb('expired_comment1').$blog_config->getTag('BLOG_COMMENT_DAYS_EXPIRY')._sb('expired_comment2');
	}elseif($blog_config->getTag('BLOG_ENABLE_COMMENTS') == 0){
		ob_end_clean();
		$entry_array['entry'] = _sb('nocomments');
	}else{
