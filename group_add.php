...
function page_content()
{
	...
	if(array_key_exists('submit',$_POST) && ($_POST['submit'] == 'Add Group' || $_POST['submit'] == 'Edit Group')){
		...
		// Try to read in the existing record or quietly fail
		$record->setTag('ID',$id);
		foreach($_POST as $key=>$val){
			if(!empty($val) && $key != 'submit' && $key != 'id'){

				// Ignore empty, submit and id
				switch($key){
					case 'superuser':
	...
	switch($mode){
		...
		// Since we were not redirected then there was an error while saving...
