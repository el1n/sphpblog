...
function page_content()
{
	...
	if(!empty($id) && array_key_exists('command',$_GET) && $_GET['command'] == 'delete'){
	...
	$t_table = new Template(TEMPLATE_DIR.'data/group_table.tpl');
	...
	for($i = $startindex;$i <= $endindex;$i++){
		...
		$t = new Template(TEMPLATE_DIR.'data/group_row.tpl');
		...
		$t->setTag('{EDIT_BUTTON}',$edit_button);

		// Edit button
		$t->setTag('{DELETE_BUTTON}',$delete_button);

		// Delete button
		$t->setTag('{EDIT_DELETE}','<span>'.$edit_button.'&nbsp;'.$delete_button.'</span>');

		// Both together
		$str = $record->getHTML($t);
		$t_table->appendTag('{MAIN}',$str);
