	...
	if(array_key_exists("info_keywords",$_POST) && array_key_exists("info_description",$_POST) && array_key_exists("info_copyright",$_POST)){
		...
		?>
			...
			<form action="info.php" method="post" name="info" name="info">
				...
				<textarea style="width: <?php global $theme_vars;
		echo($theme_vars['max_image_width']);?>px;" id="text" name="info_keywords" rows="5" cols="50" autocomplete="OFF"><?php echo($blog_config->getTag('INFO_KEYWORDS'));?></textarea><br /><br />
				
				<label for="info_description"><?php echo(_sb('info_description'));?></label><br />
				<textarea style="width: <?php global $theme_vars;
		echo($theme_vars['max_image_width']);?>px;" id="text" name="info_description" rows="5" cols="50" autocomplete="OFF"><?php echo($blog_config->getTag('INFO_DESCRIPTION'));?></textarea><br /><br />
				
				<label for="info_copyright"><?php echo(_sb('info_copyright'));?></label><br />
				<textarea style="width: <?php global $theme_vars;
		echo($theme_vars['max_image_width']);?>px;" id="text" name="info_copyright" rows="5" cols="50" autocomplete="OFF"><?php echo($blog_config->getTag('INFO_COPYRIGHT'));?></textarea><br /> <br />

				<label for="tracking_code"><?php echo(_sb('tracking_code'));?></label><br />
				<textarea style="width: <?php global $theme_vars;
		echo($theme_vars['max_image_width']);?>px;" id="text" name="tracking_code" rows="5" cols="50" autocomplete="OFF"><?php echo($blog_config->getTag('TRACKING_CODE'));?></textarea><br />
