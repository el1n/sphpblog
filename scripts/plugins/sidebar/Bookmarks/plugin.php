...
class Bookmarks extends Sidebar
{
	...
	function optionsForm()
	{
		...
		ob_start();?>
				...
				<input type="radio" name="provider" value="addthis" <?php if($provider == 'addthis'){
			echo $checked;
		}?>> AddThis
				<input type="radio" name="provider" value="addtoany" <?php if($provider == 'addtoany'){
			echo $checked;
		}?>> AddToAny
				<input type="radio" name="provider" value="socialmarker" <?php if($provider == 'socialmarker'){
			echo $checked;
		}?>> Social Marker</p>
