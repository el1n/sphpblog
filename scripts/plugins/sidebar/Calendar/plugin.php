...
class Calendar extends Sidebar
{
		...
		ob_start();?>
			...
			<form method="post" onsubmit="return validate(this)">
				...
				<select name="firstDay">
					<option label="<?php echo($sunday);?>" value="sunday"<?php if($value_firstDay == 'sunday'){
			echo ' selected';
		}?>><?php echo($sunday);?></option>
					<option label="<?php echo($monday);?>" value="monday"<?php if($value_firstDay == 'monday'){
			echo ' selected';
		}?>><?php echo($monday);?></option>
				</select><p />
	...
	function getCalendar($m,$y,$d)
	{
		...
		if($this->prefs['firstDay'] == 'sunday'){
			$date_string = mktime(0,0,0,$m,2,$y);

			// Use this for starting the calendar on Sunday
		}else{
			$date_string = mktime(0,0,0,$m,1,$y);

			//The date string we need for some info... saves space ^_^
		}
		$day_start = date('w',$date_string);

		//The number of the 1st day of the week
		...
		if(substr($entries[0],7,2) + (substr($entries[0],5,2) * 12) >= $y * 12 + $m){
			$previous_year = $y + 2000;
			$previous_month = $m;
		}

		// Don't let go past now
		if(date('m') + (date('y') * 12) <= $y * 12 + $m){
			$next_year = $y + 2000;
			$next_month = $m;
		}
		...
		//The current (column) position of the current day from the loop
		$total_days_in_month = date('t',$date_string);

		//The total days in the month for the end of the loop
		...
		$str .= '<td colspan="7" style="text-align: center">'.strftime('<a href="'.BASEURL.'index.php?y=%y&amp;m=%m&amp;d=%d">'.$this->prefs['dateFormat']).'</a></td></tr></table>';
