...
class CBlacklist
{
	...
	{
		$bReturnCode = false;
		$sContent = $GLOBALS['blog_config']->getTag('BANNED_ADDRESS_LIST');

		// sb_read_file( $sBlacklistPathFileName );
		if($sContent !== false){
...
class CBannedWords
{
	...
	{
		$bReturnCode = false;
		$sContent = $GLOBALS['blog_config']->getTag('BANNED_WORD_LIST');

		// sb_read_file( $sBannedWordsPathFileName );
		if($sContent !== false){
			$this->aEntries = explode(chr(13),trim($sContent));
			$bReturnCode = true;
		}
