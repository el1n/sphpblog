...
function read_comments($y,$m,$entry,$logged_in)
{
	...
	if($contents){
		...
		for($i = 0;$i <= count($contents) - 1;$i++){
			...
			if((($logged_in == true) and ($admin == 'no') and (CheckUserSecurity($_SESSION['username'],'MOD') == true)) or (($logged_in == true) and ($admin == 'yes'))){
...
function delete_comment($filepath)
{
	...
	// content/07/10/entry071016-093727/comments/comment071016-095416.txt.gz
	...
	if($pos !== false){
		...
		// content/07/10/entry071016-093727/comments
		...
		if(count($file_array) == 0){
			...
			if($pos !== false){
				...
				// content/07/10/entry071016-093727
				sb_delete_directory($dirpath);
			}
		}
	}
