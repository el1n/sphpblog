...
function read_entries($m,$y,$d,$logged_in,$start_entry,$category,$is_permalink = false)
{
	...
	for($i = 0;$i < count($file_array);$i++){
		...
		array_push($contents,array('entry'=>$entry_filename,'year'=>$year_dir,'month'=>$month_dir));
	}
	...
	if($contents){
		...
		for($i = 0;$i <= count($contents) - 1;$i++){
			...
			if((($logged_in == true) and ($admin == 'yes')) or (($logged_in == true) and ($admin == 'no') and (CheckUserSecurity($_SESSION['username'],'EDIT') == true) and ($blog_entry_data['CREATEDBY'] != $_SESSION['username'])) or (($logged_in == true) and ($admin == 'no') and ($blog_entry_data['CREATEDBY'] == $_SESSION['username']))){
				$entry_array['edit']['name'] = _sb('sb_edit');
				$entry_array['edit']['url'] = 'add.php?y='.$y.'&amp;m='.$m.'&amp;entry='.$entry;
			}
			if((($logged_in == true) and ($admin == 'yes')) or (($logged_in == true) and ($admin == 'no') and (CheckUserSecurity($_SESSION['username'],'DEL') == true))){
			...
			if($blog_config->getTag('BLOG_ENABLE_COMMENTS') == true){
				...
				}elseif($blog_config->getTag('BLOG_COMMENTS_MODERATION') != true){
					$comment_count = count($comment_array);
				}else{
			...
			if($blog_config->getTag('BLOG_ENABLE_VOTING') == true){
				...
				for($star_number = 1;$star_number <= 5;$star_number++){
					...
					}elseif($rating >= ($temp_ratio - .1) && $rating < $temp_ratio){
						$star_image = 'half_star.png';
					}elseif($rating >= $temp_ratio){
...
function get_entry_title($y,$m,$entry)
{
	...
	if($blog_entry == FALSE){
		...
		// For some reason we couldnt load the file
	}else{
		$title = blog_to_html($blog_entry['SUBJECT'],false,false);
