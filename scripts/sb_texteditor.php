...
function sb_editor($mode = 'entry')
{
	...
	// Value for Subject input field
	$default_content = null;

	// Value for Entry input field
	$default_tb_ping = null;

	// Only when blog_trackback_enabled = true
	$default_categories = null;

	// Field not required...
	$default_relatedlink = null;
	...
	if($mode == 'static'){
		...
		if(array_key_exists('entry',$_GET) AND empty($_POST)){
			...
			echo($entry_content);
		}elseif(array_key_exists('blog_subject',$_POST) == true){
	...
	if($mode == 'entry'){
		...
		if(array_key_exists('entry',$_GET) == true && array_key_exists('y',$_GET) == true && array_key_exists('m',$_GET) == true AND empty($_POST)){
			...
			// Create Time
			$default_time = mktime($_POST['hour'],$_POST['minute'],$_POST['second'],$_POST['month'],$_POST['day'],$_POST['year']);
