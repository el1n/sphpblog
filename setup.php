...
function page_content()
{
	...
	ob_start();?>
		...
		<form action="setup_cgi.php" method="post" name="setup" name="setup">
			...
			<a id="linkSetupLanguage" href="javascript:toggleBlock('SetupLanguage');">
				<img src="<?php global $blog_theme;
	$img_hide = 'themes/'.$blog_theme.'/images/minus.gif';
	echo($img_hide);?>" name="twisty">
	...
	echo($img_hide);?>" name="twisty">
			...
			<div id="toggleSetupGeneral" class="toggleBody"><p />
				<input type="checkbox" id="blog_enable_title" name="blog_enable_title"<?php if($blog_config->getTag('BLOG_ENABLE_TITLE') == 1){
		echo(' checked');
	}?>> <?php echo(_sb('blog_enable_title'));?><p />

				<label for="blog_header_graphic"><?php echo(_sb('blog_header'));?></label><br />
				<input type="text" name="blog_header_graphic" value="<?php echo($blog_config->getTag('BLOG_HEADER_GRAPHIC'));?>" autocomplete="OFF" size="40" style="width: <?php global $theme_vars;
	...
	echo($theme_vars['max_image_width'] - 20);?>px;"><p />
				...
				<input type="text" name="blog_email" value="<?php echo($blog_config->getTag('BLOG_EMAIL'));?>" autocomplete="OFF" size="40" style="width: <?php global $theme_vars;
	echo($theme_vars['max_image_width'] - 20);?>px;"><p />

				<label for="blog_avatar"><?php echo(_sb('blog_avatar'));?></label><br />
				<input type="text" name="blog_avatar" value="<?php echo($blog_config->getTag('BLOG_AVATAR'));?>" autocomplete="OFF" size="40" style="width: <?php global $theme_vars;
	echo($theme_vars['max_image_width'] - 20);?>px;"><p />

				<label for="blog_footer"><?php echo(_sb('blog_footer'));?></label><br />
				<input type="text" name="blog_footer" value="<?php echo($blog_config->getTag('BLOG_FOOTER'));?>" autocomplete="OFF" size="40" style="width: <?php global $theme_vars;
	echo($theme_vars['max_image_width'] - 20);?>px;"><p />

				<p>
	                        <label for="https">HTTPS Login:</label>
        	                <select name="https" id="https">
                	                <option label="NONE" value="NONE"<?php if($blog_config->getTag('HTTPS') == 'NONE'){
		echo ' selected';
	}?>>NONE</option>
                	                <option label="WARN" value="WARN"<?php if($blog_config->getTag('HTTPS') == 'WARN'){
		echo ' selected';
	}?>>WARN</option>
                        	        <option label="REQUIRE" value="REQUIRE"<?php if($blog_config->getTag('HTTPS') == 'REQUIRE'){
		echo ' selected';
	}?>>REQUIRE</option>
                        	</select>
				</p>
				...
				<input type="text" name="https_url" value="<?php echo($blog_config->getTag('HTTPS_URL'));?>" size="40" style="width: <?php global $theme_vars;
	echo($theme_vars['max_image_width'] - 20);?>px;">
				</p>
				<p>
				<input type="checkbox" id="use_emoticons" name="use_emoticons"<?php if($blog_config->getTag('USE_EMOTICONS') == 1){
		echo(' checked');
	}?>> 
				<?php print _sb("Enable Emoticons");?>
				</p>
				<p>
				<input type="checkbox" id="use_js_editor" name="use_js_editor"<?php if($blog_config->getTag('USE_JS_EDITOR') == 1){
		echo(' checked');
	}?>> 
	...
	echo($img_hide);?>" name="twisty">
			...
			<div id="toggleSetupEntries" class="toggleBody"><p />
				<input type="checkbox" id="blog_enable_voting" name="blog_enable_voting"<?php if($blog_config->getTag('BLOG_ENABLE_VOTING') == 1){
		echo(' checked');
	}?>> <?php echo(_sb('blog_enable_voting'));?><br />
				<input type="checkbox" id="blog_enable_permalink" name="blog_enable_permalink"<?php if($blog_config->getTag('BLOG_ENABLE_PERMALINK') == 1){
		echo(' checked');
	}?>> <?php echo(_sb('blog_enable_permalink'));?><br />
				<!--<input type="checkbox" id="blog_enable_print" name="blog_enable_print"<?php if($blog_config->getTag('BLOG_ENABLE_PRINT') == 1){
		echo(' checked');
	}?>> <?php echo(_sb('blog_enable_print'));?><br />-->
				...
				<input type="checkbox" id="blog_enable_capcha" name="blog_enable_capcha"<?php if($blog_config->getTag('BLOG_ENABLE_CAPCHA') == 1){
		echo(' checked');
	}?>> <?php echo(_sb('blog_enable_capcha'));?><br />
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" id="blog_enable_capcha_image" name="blog_enable_capcha_image"<?php if($blog_config->getTag('BLOG_ENABLE_CAPCHA_IMAGE') == 1){
		echo(' checked');
	}
	if(function_exists('imagecreate') == false){
		echo(' disabled');
	}?>> <?php echo(_sb('blog_enable_capcha_image'));?><br />
				<input type="checkbox" id="blog_enable_cache" name="blog_enable_cache"<?php if($blog_config->getTag('BLOG_ENABLE_CACHE') == 1){
		echo(' checked');
	}?>> <?php echo(_sb('blog_enable_cache'));?><br />
				<input type="checkbox" id="blog_send_pings" name="blog_send_pings"<?php if($blog_config->getTag('BLOG_SEND_PINGS') == 1){
		echo(' checked');
	}?>> <?php echo(_sb('blog_send_pings'));?><p />
				<label for="blog_ping_urls"><?php echo(_sb('blog_ping_urls'));?></label><br />
				<input type="text" name="blog_ping_urls" value="<?php echo($blog_config->getTag('BLOG_PING_URLS'));?>" autocomplete="OFF" size="40" style="width: <?php global $theme_vars;
	echo($theme_vars['max_image_width'] - 20);?>px;"><br />
				<br />
	...
	?>
				<input type="checkbox" id="blog_enable_start_category" name="blog_enable_start_category"<?php if($blog_config->getTag('BLOG_ENABLE_START_CATEGORY') == 1){
		echo(' checked');
	}?>> <?php echo(_sb('blog_enable_start_category'));?>
					<?php
	...
	echo($img_hide);?>" name="twisty">
		...
		echo ' selected';
					...
					<option label="<?php echo(_sb('cal_monday'));?>" value="monday"<?php if($blog_config->getTag('BLOG_CALENDAR_START') == 'monday'){
		echo ' selected';
	...
	?>
				...
				<select name="static_block_border">
					<option label="<?php echo(_sb('static_block_border'));?>" value="border"<?php if($blog_config->getTag('STATIC_BLOCK_BORDER') == 'border'){
		echo ' selected';
	}?>><?php echo(_sb('static_block_border'));?></option>
					<option label="<?php echo(_sb('static_block_noborder'));?>" value="noborder"<?php if($blog_config->getTag('STATIC_BLOCK_BORDER') == 'noborder'){
		echo ' selected';
	}?>><?php echo(_sb('static_block_noborder'));?></option>
				</select><br />
	...
	echo($img_hide);?>" name="twisty">
			...
			<div id="toggleSetupComments" class="toggleBody"><p />
				<input type="checkbox" id="blog_enable_comments" name="blog_enable_comments"<?php if($blog_config->getTag('BLOG_ENABLE_COMMENTS') == 1){
		echo(' checked');
	}?>> <?php echo(_sb('blog_enable_comments'));?><br />
				<input type="checkbox" id="blog_email_notification" name="blog_email_notification"<?php if($blog_config->getTag('BLOG_EMAIL_NOTIFICATION') == 1){
		echo(' checked');
	}?>> <?php echo(_sb('blog_email_notification'));?><br />
				...
				<table border="0" width="<?php global $theme_vars;
	echo($theme_vars['max_image_width'] - 20);?>" cellspacing="0" cellpadding="0">
...
<!--							<input type="checkbox" id="html" name="html"<?php if(in_array('html',$allowed)){
		echo(' checked');
				...
				<input type="checkbox" id="blog_comments_moderation" name="blog_comments_moderation"<?php if($blog_config->getTag('BLOG_COMMENTS_MODERATION') == 1){
		echo(' checked');
	}?>> <?php echo(_sb('blog_comments_moderation'));?><p />
	...
	echo($img_hide);?>" name="twisty">
			...
			<div id="toggleSetupCompression" class="toggleBody"><p />
				...
				<!--<input type="checkbox" id="blog_enable_gzip_txt" name="blog_enable_gzip_txt"<?php if($blog_config->getTag('BLOG_ENABLE_GZIP_TXT') == 1){
		echo(' checked');
	}
	if(extension_loaded('zlib') == false){
		echo(' disabled');
	}?>> <?php echo(_sb('blog_enable_gzip_txt'));?><br />-->
				<!--<input type="checkbox" id="blog_enable_gzip_txt" name="blog_enable_gzip_txt" disabled="disabled" <?php if($blog_config->getTag('BLOG_ENABLE_GZIP_TXT') == 1){
		echo(' checked');
	}
	if(extension_loaded('zlib') == false){
		echo(' disabled');
	}?>> <?php echo(_sb('blog_enable_gzip_txt'));?> (DEPRECATED)<br />-->
				<input type="checkbox" id="blog_enable_gzip_output" name="blog_enable_gzip_output"<?php if($blog_config->getTag('BLOG_ENABLE_GZIP_OUTPUT') == 1){
		echo(' checked');
	}
	if(extension_loaded('zlib') == false){
		echo(' disabled');
	}?>> <?php echo(_sb('blog_enable_gzip_output'));?><p />
			</div>
