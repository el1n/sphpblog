...
function directoryToArray($directory,$recursive)
{
	...
	//You can pass this array directly to addFiles()
}
...
class zipfile
{
	...
	function output($file)
	{
		...
		///666 so that every one can read it - else it is 600.
	}
...
$files = array();
$backupdirs = array();
$dirs = explode(",",$_GET['dirs']);
$allowed_dirs = array("content","config","images");
foreach($dirs as $dirname){
	if(in_array($dirname,$allowed_dirs)){
		$backupdirs[] = $dirname;
	}
}
foreach($backupdirs as $dirname){
	$tmpfiles = directoryToArray($dirname,true);
...
$zip->addFiles($newfiles);

// send headers here
$data = $zip->file();
header("Content-type: application/zip");
header("Content-length: ".strlen($data));
header("Content-disposition: attachment; filename=sphpblog_backup.zip");
print $data;
?>
